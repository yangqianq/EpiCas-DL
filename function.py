...
#Encode the DNA sequence into the onehot format
...
#Processing of epigenetic features
...
def tss_annoted(gene, pamCoord, transcript = False):
    p1p2Table = pd.read_csv(path+'/epi_reference/human_p1p2Table.txt',sep='\t', header=0, index_col=[0,1])
...
def atac_annoted(chrom, start, end, cell_line = 'Hek293t'):
    atac_bed = pybedtools.BedTool(path+'/epi_reference/%s_atac.bed'%(cell_line))
    ...
    if len(pos_atac) == 1:
        ...
        max_atac = pos_atac['atac'].argmax()
        print(pos_atac['atac'][max_atac])
        
        
def methylation_annoted(chrom,start,end,cell_line = 'Hek293t'):
    methylation_bed = pybedtools.BedTool(path+'/epi_reference/%s_methylation.bed'%(cell_line))
    pos = pybedtools.BedTool('./tempfile/%s_pos.bed'%(cell_line))                         
    ...
    if len(pos_methylation) == 1:
        ...
        max_methylation = pos_methylation['methylation'].argmax()
...
#sequence check and one-hot encoding
def seq(sequence):
    bplength = len(sequence)
    if bplength == 40:
        if sequence[30:32] != 'GG':
            print("Input sequence with incorrect form! correct format: 9bp+20bpsgRNA+3bpPAM+8bp")
            sys.exit(1)
    elif bplength == 23:
    ...
    return onehontseq,bplength

#Annotate the epigenetic features of the input sequence
...
def predict(inputs, model, class_threhold):
    ...
    models = tf.keras.models.load_model(path+'/models/'+model+'s.h5')
    modelc = tf.keras.models.load_model(path + '/models/'+model+'c.h5')
    
    efficiency_s = models.predict(inputs)[0][0]
    efficiency_c = modelc.predict(inputs)[0][0]
    class_threholds = {'CRISPRa_seq':0.10323621, 'CRISPRa_epi':0.10870079, 'CRISPRoff_seq':0.17913048, 'CRISPRoff_epi':0.25164154}
